...
data = zlib.decompress(base64.b64decode(__VAWS_ENCODED__))
if hashlib.sha256(data).hexdigest() != __VAWS_DIGEST__:
...
for step, script in request['setup']:
    ...
    run(script)
    print('__VAWS_PARITY_PROGRESS__=' + json.dumps({'phase': step,
          'elapsed_seconds': time.monotonic() - started}), file=sys.stderr, flush=True)
run(request['script'], request['cwd'])
""".replace('__VAWS_ENCODED__', repr(encoded)).replace('__VAWS_DIGEST__', repr(hashlib.sha256(data).hexdigest()))
    command = "python3 - <<'VAWS_PREPARATION_PROGRAM'\n" + program + '\nVAWS_PREPARATION_PROGRAM\n'
